...
import SessionStore, { TOOL_OUTCOME_UNKNOWN, interruptedTurnClosers, type SessionEvent } from '@deepseek-ai/dsh-session'
...
import * as NightwatchSession from '@deepseek-ai/dsh-experimental-nightwatch-session'
...
async function load(root: string): Promise<SessionEvent[]> {
  ...
  try {
    const handle = await ctx.sessionPersistence.open(EFFECT_SESSION_ID, 'read')
    try {
...
async function inspect(ctx: Context): Promise<SessionInspection> {
  const reader = await ctx.sessionPersistence.open(EFFECT_SESSION_ID, 'read')
  try {
...
async function resumeAfterReceipt(root: string, store: PdvEffectStore): Promise<{
  ...
  await ctx.plugin(NightwatchSession)
  ...
  try {
    ...
    await recordNightwatchReconciliation(ctx, handle.agent.session, {
      ...
      leaseId: 'pdv-attempt-4',
      fenceEpoch: 4,
    })
    await ctx.sessions.flush(handle.agent.session)
    await handle.dispose()
    const inspection = await inspect(ctx)
    const location = logPath(root, inspection.meta.cwd, inspection.meta.id, 'none')
    const result = {
      events: [...inspection.events],
      logBytes: await readFile(location),
      modelRequests: adapter.requests,
      operatorStatus: projectNightwatchHarness(inspection),
  ...
  it('reconciles a committed effect by call id without duplicate, drift, or stale writes', async () => {
    ...
    try {
      ...
      expect(recovered.operatorStatus).toMatchObject({
        workId: EFFECT_MISSION_ID,
        correlationId: EFFECT_CORRELATION_ID,
        failureDomain: EFFECT_FAILURE_DOMAIN,
        leaseId: EFFECT_LEASE_ID,
        fenceEpoch: EFFECT_FENCE,
        ...
        effect: {
          ...
          model: 'synthetic-v1',
          outcome: 'SUCCEEDED',
          receipt: { attemptId: 'pdv-attempt-4', leaseId: 'pdv-attempt-4', fenceEpoch: 4 },
        },
      })
